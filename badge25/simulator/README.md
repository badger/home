# Universe 2025 Badge Simulator

This simulator is archived with the Universe 2025 badge. It does not implement
the Universe 2026 runtime in `../../badge/`.
...
```bash
python badge25/simulator/badge_simulator.py badge25/apps/flappy
```
...
```bash
pip3 install pygame
python3 badge25/simulator/badge_simulator.py badge25/apps/flappy
```
...
- Uses `badge25/` as the default system root (no need for `-C badge25`)
...
To test WiFi apps:
1. Create a `badge25/secrets.py` file with your WiFi credentials and GitHub username
...
To test IR apps:
1. Run the Quest app: `python3 badge25/simulator/badge_simulator.py badge25/apps/quest`
...
Example:
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/quest
...
- `-C DIR` forces the simulator to treat `DIR` as `/system`. By default, the simulator
  uses `badge25/`, the parent of the simulator directory. Override this when apps
...
Run the menu app:
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/menu
```

Run Flappy Bird with screenshots enabled:
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/flappy --screenshots ./screenshots
```

Run at 2x scale for a smaller window:
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/life --scale 2
```

Run a specific file directly (also works):
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/gallery/__init__.py
```

Use a custom system root:
```bash
python3 badge25/simulator/badge_simulator.py -C /path/to/custom/badge /path/to/custom/badge/apps/quest
```

Clean cached files and start fresh:
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/badge --clean
```
...
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/flappy --perf
```
...
```bash
python3 badge25/simulator/badge_simulator.py badge25/apps/myapp --perf
```
