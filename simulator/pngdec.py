...
Stub implementation of pngdec for the badge simulator.
Provides PNG decoding capability using Pygame.
"""
...
try:
    ...
    raise SystemExit("Pygame is required for pngdec simulator")


class PNG:
    """PNG decoder that works with badge simulator screens."""
    ...
    def open_file(self, filename):
        ...
        try:
            ...
            raise OSError(f"Failed to open PNG file '{filename}': {e}")
    
    def get_width(self):
        """Get the width of the opened PNG image."""
        if self._surface is None:
            return 0
        return self._width
    
    def get_height(self):
        """Get the height of the opened PNG image."""
        if self._surface is None:
            return 0
        return self._height
    ...
    def decode(self, x, y, scale=1):
        """
        Decode the PNG image to the display at the specified position.
